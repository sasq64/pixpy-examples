...
# Each animation should hold a list of images
...
with os.scandir('data/knight') as it:
    for entry in it:
        if entry.name.endswith(".png") and entry.is_file():
            ...
            # assume that every pmng file contains a set of 120x80 images layed out
...
while pix.run_loop():
    ...
    if pix.is_pressed(pix.key.LEFT):
        ...
        scale = pix.Float2(2, 2)
        pos += (2,0)
    elif pix.is_pressed(pix.key.ENTER):
        attack_counter = 40

    if attack_counter > 0:
        # If we are attacking, we calculate which of the 4 attack frames to show from
        # the counter value (which goes from 40 down to 0)
        img = attack_anim[(40-attack_counter)//10]
        attack_counter -= 1
    else:
        # We calculate the current animation frame from the sprites X-position. This makes it
        # so it actually looks like the knight is using his feet to move
        frame = (pos.x // 10) % 10
        img = run_anim[frame]
